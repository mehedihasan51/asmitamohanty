...
class ChatController extends Controller
{
    ...
    public function list(): JsonResponse
    {
        ...
        $users = User::select('id', 'name', 'avatar', 'referral_code', 'coin', 'last_activity_at')
    ...
    public function search(Request $request): JsonResponse
    {
        ...
        $users = User::select('id', 'name', 'email', 'avatar', 'referral_code', 'coin', 'last_activity_at')
        ->where('id', '!=', $user_id)
    ...
    public function conversation($receiver_id): JsonResponse
    {
        ...
        $chat = Chat::query()
            ...
            ->with([ 
                'sender:id,name,avatar,referral_code,coin,last_activity_at', 
                'receiver:id,name,avatar,referral_code,coin,last_activity_at',
        ...
        $data = [
            'receiver' => User::select('id', 'name', 'avatar', 'referral_code', 'coin', 'last_activity_at')->where('id', $receiver_id)->first(),
            'sender' => User::select('id', 'name', 'avatar', 'referral_code', 'coin', 'last_activity_at')->where('id', $sender_id)->first(),
            'room' => $room,
            'chat' => $chat
        ];
    ...
    public function room($receiver_id)
    {
        ...
        $room = Room::with(['userOne:id,name,avatar,referral_code,coin,last_activity_at', 'userTwo:id,name,avatar,referral_code,coin,last_activity_at'])
        ->where(function ($query) use ($receiver_id, $sender_id) {
            $query->where('user_one_id', $receiver_id)->where('user_two_id', $sender_id);
        })->orWhere(function ($query) use ($receiver_id, $sender_id) {
            $query->where('user_one_id', $sender_id)->where('user_two_id', $receiver_id);
